...
//    }
    ...
    */
    ...
    public static function zip_folder($folder,$zip_file)
    {
        ...
        if (!is_dir($folder))
        {
            ErrorMessage::Marker("$folder is not a folder");
            return NULL;
        }

        if (!util::contains($zip_file, '.zip', false)) $zip_file .= '.zip';
        
        $folder = trim(util::trim_end($folder, '/'));
        
        //ErrorMessage::Marker(" $folder = $folder");
        
        //ErrorMessage::Marker(" Remove zip if it's already here : $zip_file");
        
        file::Delete($zip_file); 
        
        $zip_result = array();
        exec("zip -r '{$zip_file}' '$folder'",$zip_result);
        

        //ErrorMessage::Marker(" zip_result = ".print_r($zip_result,true));
        
        self::ZipRemoveSubsetOfPath($zip_file,$folder);
        
        return file_exists($zip_file);
    }


    public static function ZipRemoveSubsetOfPath($zip_filename,$path_to_remove)
    {
        
        $path_to_remove = util::toLastSlash($path_to_remove);
        
        //ErrorMessage::Marker(" containing_folder_of_files = $path_to_remove");
        
        //ErrorMessage::Marker("Remove $path_to_remove from the path of each filenames in  {$zip_filename}");
        
        
        // becasue zip does not store the first slash - if  $path_to_remove starts with slashthen remove it
        if (substr($path_to_remove,0,1) == "/") $path_to_remove = substr($path_to_remove,1);
        
        if (substr($path_to_remove,strlen($path_to_remove) -1,1) == "/" )
            $path_to_remove = substr($path_to_remove,0,strlen($path_to_remove) - 2);
                
        //ErrorMessage::Marker("true path $path_to_remove  to remove from   {$zip_filename}");
        ...
        {  
            ...
            $new_name = str_replace($path_to_remove,"",$old_name);
            if (substr($new_name,0,1) == "/") $new_name = substr($new_name,1);
            
            //ErrorMessage::Marker("rename in zip {$old_name}  {$new_name}");
            
            $rename = $zip->renameName($old_name,$new_name);
            
            //ErrorMessage::Marker("rename result =  {$rename}");
    ...
    public static function move_to_zip_folder($folder,$zip_file)
    {
        $folder = util::trim_end($folder, "/");
        ...
        exec("zip -m -r '{$zip_file}.zip' '$folder'/");
        
        
        ErrorMessage::Marker("zipfile =   $zip_file.'.zip' ");
        
        if (!file_exists($zip_file.'.zip')) 
        {
            ErrorMessage::Marker("CANT' FIND zipfile =   $zip_file.'.zip' ");
            return false;
        }
    ...
    public static function zip_file($source_file,$zip_file)
    {
        $zip_file = str_replace(".zip",'',$zip_file);

        if (!file_exists($source_file))
        {
            logger::error("$source_file does not exist");
            return NULL;
        }
        
        file::Delete($zip_file);
        
        exec("zip  '{$zip_file}.zip' '$source_file'");

        self::ZipRemoveSubsetOfPath($zip_file,dirname($zip_file)."/");
        
        return file_exists($zip_file);
    }
    

    public static function move_to_zip_file($source_file,$zip_file = null)
    {

        if (!file_exists($source_file))
        {
            ErrorMessage::Marker("$source_file does not exist");
            return NULL;
        }
        
        
        if (is_null($zip_file)) $zip_file = $source_file.".zip";
        
        if (!util::contains($zip_file, '.zip') ) $zip_file = $zip_file.".zip";
        
        
        file::Delete($zip_file);
        
        $cmd = "zip -m '{$zip_file}' '{$source_file}'";        
        $zip_result = exec($cmd);
        
        self::ZipRemoveSubsetOfPath($zip_file,dirname($zip_file)."/");
        
        if (!file_exists($zip_file))  return null;
    ...
    public static function random_filename_memory($folder = "/tmp")
    {
    	$result = $folder."/".uniqid();
    	return $result;
    }
    ...
    public static function move($source, $dest,$overwrite)
    {
        ...
        {
            echo "EERROR: source file $source does not exist\n";
            return null;
        }
        ...
        if (file_exists($dest))
        {
            echo "ERROR: dest file $dest exists\n";
            return null;
        }
    ...
    public static function ZipRemoveFirstNCharsFromPath($zip_filename,$char_count)
    {
        ErrorMessage::Marker("Remove first $char_count from {$zip_filename}");
        ...
        for($i = 0; $i < $zip->numFiles; $i++)
        {  
            $rename = $zip->renameName($zip->getNameIndex($i), substr($zip->getNameIndex($i), $char_count - 1));
        }        
